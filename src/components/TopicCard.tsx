...
const categoryColors: Record<string, string> = {
  ...
  Discussion: "bg-secondary/15 text-secondary",
  Fun: "bg-accent/15 text-accent",
  Recommendations: "bg-neon-pink/15 text-neon-pink",
  Manga: "bg-neon-purple/15 text-neon-purple",
};
...
const TopicCard = ({ topic }: TopicCardProps) => {
  return (
    <Link
      to={`/forum/${topic.id}`}
      className="block p-4 rounded-xl bg-card border border-border/40 hover:border-primary/30 transition-all duration-200 hover:bg-surface-elevated group"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${categoryColors[topic.category] || "bg-muted text-muted-foreground"}`}>
              {topic.category}
            </span>
          </div>
